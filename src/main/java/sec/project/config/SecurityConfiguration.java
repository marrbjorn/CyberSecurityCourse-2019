...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
      ...
      // with this ^ setting there will be "csrf-token as a cookie";
     //   It is can be OK, but there is also an option for HttpOnly as "false()";
 ...
 //        or if such a design is required indeed - to configure it properly and with security meanings!
        http.cors().disable();
...
/* sidenote:
there is a try to use encryption, but it is clear that md4 is not considered secure.
...
and then to implement another method:
    @Bean
    public PasswordEncoder passwordEncoder()
                                    {return new Md4PasswordEncoder();}

then will be an example of normal encryption, but good to add more tweaks.
*/
